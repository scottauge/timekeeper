...
// on Saturday, 08-Jan-2022 20:28:56 EST// $Id: TableClass.php 16 2012-03-09 10:09:43Z scott_auge $
//
...
class clsproject extends clsUtil {
  ...
  public function Create () {
    ...
    $SQL = "INSERT INTO project (ProjectRecID) VALUES (?)";
    $S = $this->DB->Connection->prepare($SQL);
    $S->bind_param("s", $ID);
    $S->execute();
    $S->close();
    $this->ProjectRecID = $ID;
  ...
  public function Delete () {
    ...
    $SQL = "DELETE FROM project WHERE ProjectRecID = ?";
    $S = $this->DB->Connection->prepare($SQL);
    $S->bind_param("s", $this->ProjectRecID);
    $S->execute();
    $S->close();
  ...
  public function Update () {
    ...
    $SQL = "UPDATE project SET ClientRecID = ?, Name = ? WHERE ProjectRecID = ?";
    $S = $this->DB->Connection->prepare($SQL);
    $S->bind_param("sss", $this->ClientRecID, $this->Name,$this->ProjectRecID);
    $S->execute();
    $S->close();
  ...
  public function FindByID ($ID) {
    ...
    $SQL = "SELECT ProjectRecID, ClientRecID, Name FROM project WHERE ProjectRecID = ?";
    $S = $this->DB->Connection->prepare($SQL);
    $S->bind_param("s", $ID);
    ...
    $S->execute();
    $S->store_result();
    $S->bind_result($this->ProjectRecID, $this->ClientRecID, $this->Name);
  ...
  public function FindByQuery ($SQL, $ArrayArgs) {
    ...
    $SQL = "SELECT ProjectRecID, ClientRecID, Name FROM " . $SQL;
    ...
    $this->_S->bind_result($this->ProjectRecID, $this->ClientRecID, $this->Name);
